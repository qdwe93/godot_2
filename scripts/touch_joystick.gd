...
# 반투명 **어두운** 색. M22 에서 흰색으로부터 뒤집었다.
#
# 예전 바닥은 근검정이라 반투명 흰색이 밝은 링으로 읽혔다. 밝은 회보라 바닥
# (#A9A3C4, 휘도 0.386) 위에서 흰색 22% 는 (0.74, 0.72, 0.81) 로 섞여 바닥과
# 거의 구분되지 않는다 — **없어지는 것이 아니라 흐려지는** 종류라 에러도 안 나고
# 테스트도 통과한다. 밝은 바닥에서 대비를 내는 방향은 위가 아니라 아래다.
#
# 스프라이트 외곽선과 같은 근검정을 쓴다. 게임 요소를 이기지 않도록 알파는 낮게 둔다.
const BASE_COLOR: Color = Color(0.078, 0.078, 0.102, 0.26)
const KNOB_COLOR: Color = Color(0.078, 0.078, 0.102, 0.50)
