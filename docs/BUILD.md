...
| 산출물 | M19 | **M22** |
|---|---:|---:|
| `index.wasm` | 37.7MB | 37.7MB (엔진, 변동 없음) |
| `index.pck` | 7.5MB (M18 까지는 1.5MB) | **9.3MB** |
| `TangtangSurvivors.exe` | 111.5MB | **113.4MB** |
| `TangtangSurvivors-debug.apk` | 62.3MB | **64.2MB** |

M22 에서 pck 가 1.8MB 더 늘었다. 바닥 타일이 256px 단색 격자에서 **512px 포석
아트**가 되면서 904바이트 -> 3.9KB 가 됐고, 스프라이트 5장에 외곽선이 구워지며
알파 경계가 늘어난 몫이다. 타일 자체는 4KB 라 **늘어난 대부분은 스프라이트**다.
...
## 검증 기록 (2026-08-15, M22 재검증)

| 플랫폼 | 결과 |
|---|---|
| Windows | 실행 후 20초 캡처 성공. **밝은 회보라 바닥 + 구운 외곽선 정상 렌더**, 피해 숫자 가독 확인 |
| 웹 | 내보내기 성공 (exit 0). 브라우저 실행은 미검증 |
| Android | 내보내기 + APK 검증 통과 (exit 0). 실기기 설치는 미검증 |
