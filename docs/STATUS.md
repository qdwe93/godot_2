...
**최종 갱신**: 2026-08-13
**현재 마일스톤**: **M7a 완료** → **다음은 M7b (보스 / 적 분리 / 성능 측정)**
...
## 바로 다음에 할 일 (M7b: 보스 / 적 분리 / 성능)

1. **보스** — 5분 시점에 **정확히 1마리**. HP 500, 크게, 검정. 웨이브 페이즈와 별개의 1회성 이벤트로 처리할 것 (페이즈 가중치에 넣으면 계속 나온다)
2. **적 분리 로직** — 지금 적들은 완전히 겹쳐 한 덩어리가 된다. 이웃과의 거리에 반비례하는 간단한 밀어내기를 `enemy.gd` 이동에 더한다
   - ⚠️ 모든 적이 모든 적을 검사하면 O(n²)다. 160마리면 25,600회/프레임. **반드시 실측**하고, 느리면 근처 적만 보는 방식(예: 자기 주변 Area2D)으로 바꾼다
3. **160마리 성능 측정** — 후반 페이즈까지 도달하는 진단으로 fps 기록. 60fps 미만이면 오브젝트 풀링 도입 검토 (D-006 재검토 트리거)
4. `test_waves`가 코드로 만든 씬을 해제하지 않아 종료 시 RID 누수 경고 1건 — 정리할 것 (무해하지만 진짜 누수를 가린다)
5. 테스트 — ① 5분에 보스가 정확히 1마리 ② 보스 스탯이 표대로 ③ 겹친 두 적이 서로 밀어내 거리가 벌어지는가 ④ 분리 로직이 목표 추적을 방해하지 않는가

**M7b 수용 기준(DoD)**: 전체 스위트 통과 + 160마리 시점 fps 기록 + 캡처로 적이 겹치지 않는 것 확인

**측정 필수 규약**: 무인 실행은 첫 레벨업에서 멈춘다. 진단 스크립트는 반드시 `level_ui.choose(0)`으로 자동 선택할 것 (devlog 010).
