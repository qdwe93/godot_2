...
**최종 갱신**: 2026-08-13
**현재 마일스톤**: **M11b 완료 (시각 위계·에셋).** 다음은 **밸런스 재측정 → 파워 곡선 재설계(M12)**

> ⚠️ **자동화 실행에는 반드시 `--auto-play`.** 게임은 타이틀에서 일시정지한 채 시작하고 레벨업에서도 멈춘다.
> ⚠️ **이전 74초 측정치는 더 이상 유효하지 않다.** 그 수치는 무기가 1종뿐이던 상태의 값이다. M11a에서 산탄·궤도구가 실제로 획득 가능해졌으므로 **재측정이 먼저다.**

---

## 한 줄 요약

**한 판의 루프가 완성됐고, M11a에서 "문서에는 있는데 실제로는 안 돌던 기능 3개"를 되살렸다.**
이동 → 적 스폰 → 자동 사격 → 처치 → 젬 드랍 → 자석 수집 → 레벨업 → 3택 → 스탯 변화 → 재개가 전부 작동하고,
이제 **무기 3종이 실제로 획득 가능**하며, 보스가 젬을 떨어뜨리고, 난이도 페이즈가 전부 도달 가능하다.

남은 가장 큰 구멍은 하나다: **플레이어 화력 상한이 1.52배인데 적 체력은 16배까지 간다.**
공격 성장이 `장갑`(쿨다운 −8%) 하나뿐이고 **데미지 업그레이드가 아예 없다.** 이게 한 판이 짧은 근본 원인이다.
...
1. **봇 진단 재측정.** 무기 3종이 실제로 붙은 상태에서 생존 시간을 다시 잰다.
   이전 74초는 무기 1종 기준이라 폐기한다. (M11a 직후 정지 상태 auto-play 실측은 **66초**였으나 이건 봇이 아니라 정지 상태 1회 값이다)
2. **데미지 업그레이드 신설 + 무기 레벨 도입.**
   현재 공격 성장은 `장갑`(쿨다운 −8% ×5 = 0.92⁵)뿐이라 **화력 상한이 1.52배**인데 적 체력은 **16배**까지 간다.
   산탄·궤도구는 아직 `max_level = 1`이라 얻으면 그걸로 끝이다.
   - `upgrade_manager._recompute_stats()`가 이미 "기본값 × 레벨 함수" 패턴이라 항목 추가가 값싸다
   - `_levels` 딕셔너리와 `get_level(id)`도 이미 있다
3. **그 다음에야 시간 곡선 조정.** 파워 상한을 안 고치고 페이즈 시작 시점만 늦추면 "지루한 75초"가 "지루한 150초"가 될 뿐이다
4. 봇의 회피 개선 (지금은 방사형 도주라 포위되면 무력)

**측정 명령** (진단 스크립트는 매번 새로 작성 — devlog 014의 규약 참고):
```powershell
Start-Process -FilePath "C:\Program Files (x86)\Godot\Godot_v4.7.1-stable_win64_console.exe" -ArgumentList '--headless','--path','C:\Workspaces\game_make\test_godot_2','--quit-after','60000','res://tests/<진단>.tscn' -RedirectStandardOutput <출력파일> -NoNewWindow
```
파이프라인(`Select-String`)으로 받으면 버퍼링되어 진행이 안 보인다. **파일로 직접 리다이렉트할 것.**
