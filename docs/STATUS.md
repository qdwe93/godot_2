...
**최종 갱신**: 2026-08-13
**현재 마일스톤**: **M0~M10 1차 완주.** 다음은 **밸런싱 2차** (아래 "바로 다음에 할 일")

> ⚠️ **자동화 실행에는 반드시 `--auto-play`.** 게임은 타이틀에서 일시정지한 채 시작하고 레벨업에서도 멈춘다.
> ⚠️ **1회 플레이가 약 75초로 목표(10~15분)에 크게 못 미친다.** M10에서 원인을 특정했고 조치 순서를 아래에 적어뒀다.

> ⚠️ **자동화 실행에는 반드시 `--auto-play`를 붙일 것.** M8b부터 게임은 타이틀에서 일시정지한 채 시작하고, 레벨업에서도 멈춘다. 이 플래그가 둘 다 건너뛴다 (CLAUDE.md 참고).
...
## 바로 다음에 할 일 (밸런싱 2차) — 순서가 중요하다

1. **`tests/test_waves.gd`를 불변식 기반으로 고친다.** 지금은 특정 시각(29.9초, 60초, 300초)과 그 시점의 값을 하드코딩해서, 페이즈 타이밍을 바꾸면 무조건 3케이스가 깨진다. 이걸 먼저 고치지 않으면 2번을 할 수 없다
   - 값 대신 불변식을 검사할 것: 모든 `i`에 대해 `start_time[i]`에서 인덱스 `i`, `start_time[i]-0.1`에서 `i-1` / `spawn_interval` 단조 감소 / `max_enemies`·`health_multiplier` 단조 증가 / `weights`의 id가 실재하는지
   - **밸런스 테스트가 밸런스 수치를 하드코딩하면 튜닝 반복이 막힌다** (devlog 014)
2. 페이즈 시작 시점을 늘린다. 권장: `0 / 75 / 150 / 240 / 360 / 480 / 600 / 720 / 840 / 960`초 (강도는 그대로, 도달 시점만 지연)
3. 봇으로 재측정 → 목표 10~15분
4. 여전히 짧으면 초반 페이즈의 `spawn_interval`·`max_enemies` 자체를 완화
5. 봇의 회피를 개선하면 측정이 실제에 가까워진다 (지금은 방사형 도주라 포위되면 무력)
