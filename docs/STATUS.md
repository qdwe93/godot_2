...
**최종 갱신**: 2026-08-13
**현재 마일스톤**: **M6 완료** (M6a + M6b) → **다음은 M7 (웨이브와 난이도)**
...
## 바로 다음에 할 일 (M7: 웨이브와 난이도)

**목표**: 시간이 갈수록 적이 강해져 실제로 죽을 수 있는 게임이 된다.

> ⚠️ **지금 가장 큰 구멍이다.** 처치 속도(1.0마리/초)가 스폰 속도(0.67마리/초)를 앞서 적이 쌓이지 않는다. 그래서 ① 플레이어가 죽지 않고 ② 젬이 멀리 떨어져 레벨업도 거의 안 일어난다. **M5·M6에서 만든 성장 시스템이 실제로는 거의 작동하지 않고 있다.** 이걸 고쳐야 나머지가 의미를 갖는다.

1. **난이도 매니저** — 경과 시간에 따라 스폰 간격·동시 최대 적 수·적 종류를 조정. GDD 8절 표가 출발점이지만 **실측이 우선**이다. 표대로 해도 부족하면 다음 중 하나 이상을 쓴다:
   - 스폰 간격을 표보다 더 공격적으로 단축
   - 적 HP를 시간에 따라 증가 (처치 시간을 늘려 적이 쌓이게)
   - 한 번에 여러 마리 묶음 스폰
2. **적 종류 3종** (GDD 5절): 빠른 적(2분~), 탱커(4분~). `enemy.tscn`을 상속하거나 `@export`로 스탯만 바꾼 변형으로
3. **보스 1종** (5분): HP 500, 크게, 검정
4. **적 분리 로직** — 지금은 적들이 완전히 겹쳐 한 덩어리가 된다. 밀도가 올라가면 눈에 띈다. 서로 밀어내는 간단한 분리력을 넣되 **성능을 실측**할 것
5. **성능 측정** — 적 150마리 상황에서 fps 확인. 60 미만이면 오브젝트 풀링 도입 검토 (D-006 재검토 트리거)
6. 테스트 — ① 시간대별 스폰 간격이 표대로인가 ② 시간대별로 새 적 종류가 등장하는가 ③ 5분에 보스가 정확히 1마리 나오는가 ④ 동시 적 수 상한이 지켜지는가

**M7 수용 기준(DoD)**: 테스트 통과 + **실제로 플레이어가 죽는다는 것을 실측 로그로 증명** + 적 150마리에서 fps 측정값 기록

**검증 팁**: 사람이 조작하지 않으면 플레이어는 가만히 서 있으므로 실제 난이도와 다르다. 진단 스크립트로 여러 조건(정지 / 무기 해제)에서 생존 시간을 측정해 비교할 것.
